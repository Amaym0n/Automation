...
def get_access_token(
    conf: dict[str, Any], test_params: Union[dict[str, Any], dict]
) -> str:
    """Фикстура для получения Access-токена"""
    user = (
        User.SUPERVISOR
        if not test_params.get("user", False)
        else test_params.get("user")
    )
    with allure.step(f"Получение Access-токена для {user}"):
        data = {"username": user.get_username(), "password": user.get_password()}
        response = ApiClient(conf=conf).post(
            path=Urls.login_for_access_token.url,
            data=data,
            headers=Headers.DATA_TYPE.value,
        )
...
@pytest.fixture(scope="function")
def create_multiple_headers(
    test_params: Union[dict[str, Any], dict], get_access_token: str
) -> Headers:
    """Создание мульти-хэдера"""
    with allure.step("Создание мульти-хэдера"):
        headers = (
            Headers.JSON_TYPE.jwt_headers(jwt_token=get_access_token)
