...
class RestClient:
    ...
    def get(self, path: str, params: Optional[dict[str, Any]] = None, data: Any = None,
            json: Optional[dict[str, Any]] = None, headers: Optional[Headers] = Headers.NONE_TYPE,
            files: Optional = None, status_code: HTTPStatus = HTTPStatus.OK, verify: bool = False) -> Response:
        """ Формирование и отправка GET-запроса """
        with allure.step(title=f'GET-запрос по url -> {self.basic_path}{path}'):
            response = requests.get(url=f'{self.basic_path}{path}', params=params, data=data, json=json,
                                    headers=headers.value, cookies=self.cookies, files=files, timeout=300,
                                    verify=verify)
            self._check_status_code(response_status=response, expected_status_code=status_code)
        return response

    def post(self, path: str, params: Optional[dict[str, Any]] = None, data: Any = None,
             json: Optional[dict[str, Any]] = None, headers: Optional[Headers] = Headers.NONE_TYPE,
             files: Optional = None, status_code: HTTPStatus = HTTPStatus.OK, verify: bool = False) -> Response:
    ...
    def delete(self, path: str, params: Optional[dict[str, Any]] = None, data: Any = None,
               json: Optional[dict[str, Any]] = None, headers: Optional[Headers] = Headers.NONE_TYPE,
               files: Optional = None, status_code: HTTPStatus = HTTPStatus.OK, verify: bool = False) -> Response:
        """ Формирование и отправка DELETE-запроса """
        with allure.step(title=f'DELETE-запрос по url -> {self.basic_path}{path}'):
            response = requests.delete(url=f'{self.basic_path}{path}', params=params, data=data, json=json,
                                       headers=headers.value, cookies=self.cookies, files=files, timeout=300,
                                       verify=verify)
            self._check_status_code(response_status=response, expected_status_code=status_code)
        return response
    ...
    @staticmethod
    def _check_status_code(response_status: Response, expected_status_code: HTTPStatus = HTTPStatus.OK) -> None:
